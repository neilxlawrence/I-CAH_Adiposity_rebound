...
```{r, extract only those with height and separate by sex}
ar_data_both_sexes_with_interpolated_height <-
  subset(ar_data, !is.na(interpolated_height))

ar_data_male_with_interpolated_height <-
  subset(ar_data, 
         !is.na(interpolated_height) &
         sex_1_for_M_2_for_F==1)

ar_data_fema_with_interpolated_height <-
  subset(ar_data, 
         !is.na(interpolated_height) &
...
min_age <- 0.2 #just if you're running chunks selectively
maximum_age_to_use_values <- 20  # Example range
max_age <- 20 #just if you're running chunks selectively
degrees_of_freedom_values <- 5  # Example range
df <- 5 #just if you're running chunks selectively
minimum_number_of_visits <- 6
min_visits <- 6


this_file_variable <- "interpolated_height"
```
...
when just restricting by visit the minimum needed was about 44 to fit sitar through height, which is not great at all
...
```{r, both_sexes fit the sitar model for height using different restrictions on age df and number of visits}
...
################### 
  ...
  if (nrow(ar_data_both_sexes_restricted) == 0) {
    ...
    list_of_both_sexes_fitting_results_height[[]] <- 
      "No data"
        next
      }
  ...
  list_of_both_sexes_sitar_models_height[[name_of_model_restrictions]] <- 
      sitar_model_fit

  # Extract fixed effects
  #lets use age and then calculate the ln rather than the other way around
  fixed_effects_fit_data_frame_height <- 
    data.frame(age = seq(0.1, 20, by = 0.1))
  
#  fixed_effects_fit_data_frame_height <- 
#    data.frame(ln_post_conceptual_age = seq(-0.3, 3.05, by = 0.01))
  #calculate ln_post_conceptual_age
  fixed_effects_fit_data_frame_height$ln_post_conceptual_age <-
    log(fixed_effects_fit_data_frame_height$age + 0.75)
  
  #back calculate age
#  fixed_effects_fit_data_frame_height$age <-
#    exp(fixed_effects_fit_data_frame_height$ln_post_conceptual_age) - 0.75
  
  #predict fixed effects using a level of zero
  fixed_effects_fit_data_frame_height$height <- 
    as.numeric(predict(sitar_model_fit, newdata = fixed_effects_fit_data_frame_height, level = 0))
  
  # Store fixed effects
  list_of_both_sexes_overall_fixed_effects_height[[name_of_model_restrictions]] <- 
      fixed_effects_fit_data_frame_height
  ...
  list_of_both_sexes_random_effects_height[[name_of_model_restrictions]] <- 
      sitar_model_random_effects
  ...
  individual_patient_random_effects_for_curves_height <- data.frame()
...
#      exp(single_person_frame$ln_post_conceptual_age) - 0.75
  single_person_frame$ln_post_conceptual_age <-
    ...
    individual_patient_random_effects_for_curves_height <- 
      rbind(individual_patient_random_effects_for_curves_height, single_person_frame)
      
  }
  
  #store the large frame with random effects predictions for curves
  list_of_both_sexes_individual_patient_random_effects_for_curves_height[[name_of_model_restrictions]] <- 
      individual_patient_random_effects_for_curves_height
  ...
  ar_data_both_sexes_restricted_random_effect_predictions$sitar_prediction_height <-
    predict(
      sitar_model_fit, 
  ...
  list_of_both_sexes_patient_data_with_sitar_predictions_height[[name_of_model_restrictions]] <- 
      ar_data_both_sexes_restricted_random_effect_predictions
...
list_of_both_sexes_fitting_results_height[[name_of_model_restrictions]] <- 
    fit_status
...
if (!is.null(error_msg)) {
  print(fit_status)
  print(error_msg)
  list_of_both_sexes_error_messages_height[[name_of_model_restrictions]] <- 
      error_msg
}

#save the lists each time so we can review them and mess with them on another computer
save(list_of_both_sexes_sitar_models_height,   
     file = paste0("ar_data_files_to_load/list_of_both_sexes_sitar_models_height.Rdata"))
save(list_of_both_sexes_overall_fixed_effects_height,   
     file = paste0("ar_data_files_to_load/list_of_both_sexes_overall_fixed_effects_height.Rdata"))
save(list_of_both_sexes_random_effects_height,   
     file = paste0("ar_data_files_to_load/list_of_both_sexes_random_effects_height.Rdata"))
save(list_of_both_sexes_patient_data_with_sitar_predictions_height,   
     file = paste0("ar_data_files_to_load/list_of_both_sexes_patient_data_with_sitar_predictions_height.Rdata"))
save(list_of_both_sexes_individual_patient_random_effects_for_curves_height,   
     file = paste0("ar_data_files_to_load/list_of_both_sexes_individual_patient_random_effects_for_curves_height.Rdata"))
save(list_of_both_sexes_fitting_results_height,   
     file = paste0("ar_data_files_to_load/list_of_both_sexes_fitting_results_height.Rdata"))
save(list_of_both_sexes_error_messages_height,   
     file = paste0("ar_data_files_to_load/list_of_both_sexes_error_messages_height.Rdata"))
...
```{r, male fit the sitar model for height using different restrictions on age df and number of visits}
...
###################  
  ...
  if (nrow(ar_data_male_restricted) == 0) {
    cat("Skipping: No data left after filtering.\n")
    list_of_male_fitting_results_height[[]] <- 
      "No data"
        next
      }
  ...
  # Store the successfully fitted model
  list_of_male_sitar_models_height[[name_of_model_restrictions]] <- 
      sitar_model_fit
...
#    exp(fixed_effects_fit_data_frame_height$ln_post_conceptual_age) - 0.75
  ...
  # Store fixed effects
  list_of_male_overall_fixed_effects_height[[name_of_model_restrictions]] <- 
      fixed_effects_fit_data_frame_height
  ...
  # Store random effects
  list_of_male_random_effects_height[[name_of_model_restrictions]] <- 
      sitar_model_random_effects
...
#      exp(single_person_frame$ln_post_conceptual_age) - 0.75
  ...
  #store the large frame with random effects predictions for curves
  list_of_male_individual_patient_random_effects_for_curves_height[[name_of_model_restrictions]] <- 
      individual_patient_random_effects_for_curves_height
  ...
  #add the sitar individual patient level predictions by patient
  ar_data_male_restricted_random_effect_predictions$sitar_prediction_height <-
    predict(
      sitar_model_fit, 
  ...
  #store the original data and predictions outside of the loop
  list_of_male_patient_data_with_sitar_predictions_height[[name_of_model_restrictions]] <- 
      ar_data_male_restricted_random_effect_predictions
...
#store whether the model failed or succeeded  
list_of_male_fitting_results_height[[name_of_model_restrictions]] <- 
    fit_status
...
if (!is.null(error_msg)) {
  print(fit_status)
  print(error_msg)
  list_of_male_error_messages_height[[name_of_model_restrictions]] <- 
      error_msg
}

#save the lists each time so we can review them and mess with them on another computer
save(list_of_male_sitar_models_height,   
     file = paste0("ar_data_files_to_load/list_of_male_sitar_models_height.Rdata"))
save(list_of_male_overall_fixed_effects_height,   
     file = paste0("ar_data_files_to_load/list_of_male_overall_fixed_effects_height.Rdata"))
save(list_of_male_random_effects_height,   
     file = paste0("ar_data_files_to_load/list_of_male_random_effects_height.Rdata"))
save(list_of_male_patient_data_with_sitar_predictions_height,   
     file = paste0("ar_data_files_to_load/list_of_male_patient_data_with_sitar_predictions_height.Rdata"))
save(list_of_male_individual_patient_random_effects_for_curves_height,   
     file = paste0("ar_data_files_to_load/list_of_male_individual_patient_random_effects_for_curves_height.Rdata"))
save(list_of_male_fitting_results_height,   
     file = paste0("ar_data_files_to_load/list_of_male_fitting_results_height.Rdata"))
save(list_of_male_error_messages_height,   
     file = paste0("ar_data_files_to_load/list_of_male_error_messages_height.Rdata"))
...
```{r, fema fit the sitar model for height using different restrictions on age df and number of visits}
...
###################  
  ...
  if (nrow(ar_data_fema_restricted) == 0) {
    cat("Skipping: No data left after filtering.\n")
    list_of_fema_fitting_results_height[[]] <- 
      "No data"
        next
      }
  ...
  # Store the successfully fitted model
  list_of_fema_sitar_models_height[[name_of_model_restrictions]] <- 
      sitar_model_fit
...
#    exp(fixed_effects_fit_data_frame_height$ln_post_conceptual_age) - 0.75
  ...
  # Store fixed effects
  list_of_fema_overall_fixed_effects_height[[name_of_model_restrictions]] <- 
      fixed_effects_fit_data_frame_height
  ...
  # Store random effects
  list_of_fema_random_effects_height[[name_of_model_restrictions]] <- 
      sitar_model_random_effects
...
#      exp(single_person_frame$ln_post_conceptual_age) - 0.75
  ...
  #store the large frame with random effects predictions for curves
  list_of_fema_individual_patient_random_effects_for_curves_height[[name_of_model_restrictions]] <- 
      individual_patient_random_effects_for_curves_height
  ...
  #add the sitar individual patient level predictions by patient
  ar_data_fema_restricted_random_effect_predictions$sitar_prediction_height <-
    predict(
      sitar_model_fit, 
  ...
  #store the original data and predictions outside of the loop
  list_of_fema_patient_data_with_sitar_predictions_height[[name_of_model_restrictions]] <- 
      ar_data_fema_restricted_random_effect_predictions
...
#store whether the model failed or succeeded  
list_of_fema_fitting_results_height[[name_of_model_restrictions]] <- 
    fit_status
...
if (!is.null(error_msg)) {
  print(fit_status)
  print(error_msg)
  list_of_fema_error_messages_height[[name_of_model_restrictions]] <- 
      error_msg
}

#save the lists each time so we can review them and mess with them on another computer
save(list_of_fema_sitar_models_height,   
     file = paste0("ar_data_files_to_load/list_of_fema_sitar_models_height.Rdata"))
save(list_of_fema_overall_fixed_effects_height,   
     file = paste0("ar_data_files_to_load/list_of_fema_overall_fixed_effects_height.Rdata"))
save(list_of_fema_random_effects_height,   
     file = paste0("ar_data_files_to_load/list_of_fema_random_effects_height.Rdata"))
save(list_of_fema_patient_data_with_sitar_predictions_height,   
     file = paste0("ar_data_files_to_load/list_of_fema_patient_data_with_sitar_predictions_height.Rdata"))
save(list_of_fema_individual_patient_random_effects_for_curves_height,   
     file = paste0("ar_data_files_to_load/list_of_fema_individual_patient_random_effects_for_curves_height.Rdata"))
save(list_of_fema_fitting_results_height,   
     file = paste0("ar_data_files_to_load/list_of_fema_fitting_results_height.Rdata"))
save(list_of_fema_error_messages_height,   
     file = paste0("ar_data_files_to_load/list_of_fema_error_messages_height.Rdata"))
...
```{r, plot the height fixed and random effects fits for the model}
#create three separate csv files to add results to
fit_statistics_of_both_sexes_models_height <- data.frame()
fit_statistics_of_male_models_height <- data.frame()
fit_statistics_of_fema_models_height <- data.frame()
...
list_of_combinations <- names(list_of_both_sexes_sitar_models_height)
#change that list by sex
if (grepl(x=lists_to_loop_around, pattern="male")){
  list_of_combinations <- names(list_of_male_sitar_models_height)
}    
if (grepl(x=lists_to_loop_around, pattern="fema")){
  list_of_combinations <- names(list_of_fema_sitar_models_height)
}    
...
underlying_sitar_model <- (list_of_both_sexes_sitar_models_height[[name_of_data]])
fixed_effects_to_plot <- (list_of_both_sexes_overall_fixed_effects_height[[name_of_data]])
random_effects_to_plot <- (list_of_both_sexes_individual_patient_random_effects_for_curves_height[[name_of_data]])

if (grepl(x=lists_to_loop_around, pattern="male")){
  underlying_sitar_model <- (list_of_male_sitar_models_height[[name_of_data]])
  fixed_effects_to_plot <- (list_of_male_overall_fixed_effects_height[[name_of_data]])
  random_effects_to_plot <- (list_of_male_individual_patient_random_effects_for_curves_height[[name_of_data]])
}

if (grepl(x=lists_to_loop_around, pattern="fema")){
  underlying_sitar_model <- (list_of_fema_sitar_models_height[[name_of_data]])
  fixed_effects_to_plot <- (list_of_fema_overall_fixed_effects_height[[name_of_data]])
  random_effects_to_plot <- (list_of_fema_individual_patient_random_effects_for_curves_height[[name_of_data]])
}
...
point_colour_by_sex <-
  "black"
ar_data_points_to_plot_for_height <-
  ar_data_both_sexes_with_interpolated_height
...
plot_fixed_and_random_sitar_effect_fits_height <-
  ggplot() +
#plot the actual data points in the background
  geom_point(
    data=subset(ar_data_points_to_plot_for_height, age_to_use>=min_age & age_to_use<=max_age),
    aes(
      x=age_to_use,
      y=interpolated_height
    ),
...
dir.create(
  paste0("plot_fixed_and_random_sitar_effect_fits_height_",
...
# Save the plot as a png file
  ggsave(filename = 
           paste0("SITAR_fit_", 
                  name_of_data,
                  ".png"),
         path = paste0("plot_fixed_and_random_sitar_effect_fits_height_",
         lists_to_loop_around),
         plot = plot_fixed_and_random_sitar_effect_fits_height,
         device = "png",
         width = 6,
         height = 3,
         limitsize = FALSE)
...
if (lists_to_loop_around=="both_sexes"){
  fit_statistics_of_both_sexes_models_height <-
    rbind(fit_statistics_of_both_sexes_models_height, fit_statistics_of_model)
  write.csv(
    fit_statistics_of_both_sexes_models_height,
    "fit_statistics_of_both_sexes_models_height.csv",
    row.names=F
  )
}
#if we are doing male we add it to the male csv file  
if (lists_to_loop_around=="male"){
  fit_statistics_of_male_models_height <-
    rbind(fit_statistics_of_male_models_height, fit_statistics_of_model)
  write.csv(
    fit_statistics_of_male_models_height,
    "fit_statistics_of_male_models_height.csv",
    row.names=F
  )
}
#if we are doing fema we add it to the fema csv file  
if (lists_to_loop_around=="fema"){
  fit_statistics_of_fema_models_height <-
    rbind(fit_statistics_of_fema_models_height, fit_statistics_of_model)
  write.csv(
    fit_statistics_of_fema_models_height,
    "fit_statistics_of_fema_models_height.csv",
    row.names=F
  )
}
...
all_male_fit_statistics_height <- data.frame()

for (each_model in names(list_of_male_sitar_models_height)){

model <- list_of_male_sitar_models_height[[each_model]]
...
all_male_fit_statistics_height <- rbind(all_male_fit_statistics_height, fit_statistics)
write.csv(all_male_fit_statistics_height, "all_male_fit_statistics_height.csv", row.names=F)

}

all_fema_fit_statistics_height <- data.frame()

for (each_model in names(list_of_fema_sitar_models_height)){

model <- list_of_fema_sitar_models_height[[each_model]]
...
all_fema_fit_statistics_height <- rbind(all_fema_fit_statistics_height, fit_statistics)
write.csv(all_fema_fit_statistics_height, "all_fema_fit_statistics_height.csv", row.names=F)

}

all_both_sexes_fit_statistics_height <- data.frame()

for (each_model in names(list_of_both_sexes_sitar_models_height)){

model <- list_of_both_sexes_sitar_models_height[[each_model]]
...
all_both_sexes_fit_statistics_height <- rbind(all_both_sexes_fit_statistics_height, fit_statistics)
write.csv(all_both_sexes_fit_statistics_height, "all_both_sexes_fit_statistics_height.csv", row.names=F)
}
